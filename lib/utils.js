...
class streamMonitor {
   ...
   *
   * @param {Stream} st
   * @param  {...Array[]} args array of [name:String, cb:Function]
   */
  constructor(st, ...args) {
    this.st = st;
    this.eventcbs = [...args];

    this.eventcbs.forEach(evt => {
      st.on(...evt);
    });
  }
  ...
  stop() {
    this.eventcbs.forEach(evt => {
      try {
        this.st.off(...evt);
        delete this.st;
        delete this.eventcbs;
      } catch (err) {}
    });
  }
}

function strsignal_win(num) {
  switch (num) {
    case 22:
      return "SIGABRT";
    case 8:
...
function strsignal(num) {
  return (
    "SIG" +
    spawnSync("kill", ["-l", String(num)], {
      encoding: "utf8",
      shell: true
    }).stdout.slice(0, -1)
  );
...
var utils = (module.exports = {
  strsignal: process.platform === "win32" ? strsignal_win : strsignal,
  ...
  /**
   * @param {string} file     Executable file to search its path
   * @param {string|string[]} Environment variable(s) to search
   * @returns {string} full path of the file, empty string if none found
   */
  which: (file, envname = "") => {
    // if windows, make sure .exe extension is present
    const isWin = process.platform === "win32";
    if (isWin && !file.endsWith(".exe")) file = file + ".exe";

    // for each environment variable name, check for presense
    if (envname) {
      let p;
      if (
        (Array.isArray(envname) ? envname : [envname])
    ...
    // look for the file on the system PATH
...
function parseStreamDump(dump) {
  ...
  if (lastdata.endsWith(")")) {
    ...
    stream.codecdata.push(lastdata)
  }
